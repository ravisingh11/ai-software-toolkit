...
| AI Engineering Review Adapter, AI QA Review Adapter, AI Security Review Adapter, AI Repository Standards Review Adapter | AI engineering, QA, security, and repository-standards review | None (the adapter command's own credentials are repository secrets) | Copy `workflows/ai-pr-review.yml`, set `AI_REVIEW_COMMAND` to a repository-owned adapter that writes each role's result JSON, and verify the four `AI ... Review` checks on a representative PR. Advisory-only; cannot be promoted. |
| Repository Soak Command | Runtime soak | None | Copy `workflows/soak.yml`, set `SOAK_COMMAND`, and verify the scheduled `Soak Check`; an evidence contract until a consumer proves it. |
